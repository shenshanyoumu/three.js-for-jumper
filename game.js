/**
 * Game对象挂载到window上，作为全局对象使用
 */
var Game = function() {
  // 基本参数
  this.config = {
    isMobile: false, // 在移动端的交互动作使用"touch"
    background: 0x282828, // 游戏背景颜色
    ground: -1, // 地面y坐标，之所有设置为-1是为了在场景中完整显示骰子而不是被剪切一半
  ...
  // Render有很多种，一般三维利用WebGLRenderer渲染器；antialias抗锯齿会使得渲染性能下降一些
  this.renderer = new THREE.WebGLRenderer({ antialias: true });

  this.cubes = []; // 方块数组
  this.cubeStat = {
    nextDir: "" // 下一个方块相对于当前方块的方向: 'left' 或 'right'，这个变量主要觉得jumper的起跳方向
  };

  // 注意了！！如果相机没有旋转操作，则一直Z轴负方向观察，其右边为X轴正向，上方为Y轴正向
  this.jumperStat = {
    ready: false, // 鼠标按完没有
  ...
  // 判断是否在方块上
  this.falledStat = {
    location: -1, // jumper所在的位置，默认值-1 表示在当前立方体
...
Game.prototype = {
  ...
  _checkInCube: function() {
    if (this.cubes.length > 1) {
      ...
      var distanceS, // jumper和跳出方块的中心距离
        distanceL; // jumper和跳向方块的中心距离

      // 判断跳出方块和跳向方块的方向来确定计算距离的坐标轴
      ...
      // should的值是jumper在立方体上移动的最大距离(由于jumper只能正交性移动，因此不考虑对角距离)
  ...
  _createCube: function() {
    ...
    if (this.cubes.length) {
      ...
      // 初始化是都在世界坐标系的坐标原点
      //注意，其实场景中的所有立方体都没有发生过旋转操作，只是简单在XZ平面上相对于上一个立方体进行随机位移
  ...
  // 核心函数，而且使用RequestFrameAnimation()周期性调用，不然场景不会显示，主要原因是canvas绘画是immediate mode，帧缓冲在一次绘制后被自动清除
  _render: function() {
  ...
  // 设置相机初始位置为(100,100,100)，并观察坐标原点。
  _setCamera: function() {
    this.camera.position.set(100, 100, 100);
    this.camera.lookAt(this.cameraPos.current);
  },

  // 渲染器必须加载在DOM节点上，其实所有元素无论是否shadow DOM都需要挂载
